...
Router.get("/file-data/:id", (req, res) => {
  db.file.findOne({ where: { Id: req.params.id } }).then((file) => {
...
Router.get("/first-last/:isfirst/:folderid", (req, res) => {
  db.file
    .findAll({ order: ["Name"], where: { FolderId: req.params.folderid }, include: [{ model: db.folder }] })
    .then((files) => {
